...
export default function Navbar({settings}: {settings?: SiteSetting }) {
  return (
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        {settings?.logo?.url ? (
          <img src={`${process.env.NEXT_PUBLIC_STRAPI_URL}${settings.logo.url}`}
            alt={settings.companyName || "Logo"} className="h-9 w-auto"/>
          ) : (
